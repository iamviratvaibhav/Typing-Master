    ...
    return (
        ...
        <nav className={` flex justify-between items-center px-8 py-4 bg-black text-white border-b border-zinc-800 fixed top-0 w-full z-50 transition duration-300 px-[7vw] md:px-[7vw] lg:px-[10vw] ${isScroll ? "bg-[#050414] bg-opacity-50 backdrop-blur-md shadow-md" : "bg-transparent"
            ...
            <ul className="flex gap-6 text-sm font-medium">
                ...
                {/* <House id="home" className="hover:text-teal-400 cursor-pointer"/>
                <MdOutlinePeopleAlt id='multiplayer' className="hover:text-teal-400 cursor-pointer size-6"/>
                <MdOutlineLeaderboard id='leaderboard' className="hover:text-teal-400 cursor-pointer size-6"/>
                <CgProfile id='profile' className="hover:text-teal-400 cursor-pointer size-6"/>
                 */}
            ...
            <ul className={`flex gap-6 text-sm font-medium`}>
                ...
                {menuIteam.map((item)=>(
                        <li key={item.id} className={`cursor-pointer hover:text-teal-400 ${activeSection === item.id ? "text-teal-400 cursor-pointer" : "none"}`}>
                            <button onClick={() => handleMenuItemClick(item.id)}>{item.lable}</button>
